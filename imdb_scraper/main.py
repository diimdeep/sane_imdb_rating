...
def parse_title_year(soup):
    title = soup.select('h1[data-testid="hero-title-block__title"]')[0].text.strip()
    #soup.select('h1[data-testid="hero-title-block__metadata"]')[0].text.strip()
    return title, 'todo'

def parse_rating(soup):
    return soup.select('div[data-testid="hero-title-block__aggregate-rating__score"]')[0].select('span')[0].text
